...
function chargeCategorie($catId,$coursId=NULL) {
	...
	$results = $mysqli->query("SELECT `Categorie-Evaluations`.* FROM `Categorie-Evaluations` WHERE  `Categorie-Evaluations`.`index`=$catId");
	$params = $results->fetch_assoc();
...
class categorie {
	...
	function chargeResultats($listeEtudiants) {
		...
		if($this->type()=='equipes') {
			...
			$results = $mysqli->query('SELECT `prescription`, `evaluateur`, `evalue`, `retard`, `notes` FROM `Equipes-evaluations` WHERE `prescription` IN ('.implode(',',$liste).')');
...
class categorieDevoirs extends categorie {
	...
	function __construct($id,$data=NULL) {
		...
		if(!is_null($data)) {
			...
			$results = $mysqli->query("SELECT `Categorie-Evaluations`.* FROM `Categorie-Evaluations` WHERE  `Categorie-Evaluations`.`index`=$id");
			$this->defaultParams = $results->fetch_assoc();
			$results->free();
		}
		
		if($this->defaultParams['devoirs-params']!=0) {
			$results = $mysqli->query("SELECT `Devoirs-parametres`.* FROM `Devoirs-parametres` WHERE `Devoirs-parametres`.`index`=".$this->defaultParams['devoirs-params']);
			$this->devoirsParams = $results->fetch_assoc();
			$results->free();
		}
		
		if($this->defaultParams['theorie-params']!=0) {
			$results = $mysqli->query("SELECT `Theorie-parametres`.* FROM `Theorie-parametres` WHERE `Theorie-parametres`.`index`=".$this->defaultParams['theorie-params']);
			$this->theorieParams = $results->fetch_assoc();
			$results->free();
		}
    }
	...
	private function getParams($nom,$prescriptionIndex,$type="devoirs") {
		...
		if($prescriptionIndex!==FALSE) {
			if(!isset($this->prescriptionsParams[$type][$prescriptionIndex])) {
				GLOBAL $mysqli;
				if($type=='devoirs') {$results = $mysqli->query("SELECT * FROM `Devoirs-parametres` WHERE `prescription`=$prescriptionIndex");}
				else {$results = $mysqli->query("SELECT * FROM `Theorie-parametres` WHERE `prescription`=$prescriptionIndex");}
